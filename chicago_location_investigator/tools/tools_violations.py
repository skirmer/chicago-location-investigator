...
def search_violations(location:LocationInput, start_date:str = None,  end_date: str = None):
    """Search for building code violations at a specific address or within coordinate boundaries with optional date filtering.
    ...
    print(f"Retrieving building violations at {location}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not violations:
                return f"No violations found at {location.value} during date range selected."

            # Format as string summary to make it easier for the LLM to understand
            summary = f"Found {len(violations)} violation(s) at {location.value} during date range selected:\n\n"
            for v in violations:
